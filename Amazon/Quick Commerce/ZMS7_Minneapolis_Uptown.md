---
aliases:
  - Minneapolis_Uptown
site_code: ZMS7
launch_date:
pin: Uptown
address: 2020 28th Street East, Suite TBD, Minneapolis, MN 55407
city: Minneapolis
state: MN
deal_type: New Lease
business_unit: QC
status: Surveying
local_broker:
loi_final: "No"
loi_prepared: "Yes"
site_selection_date: ""
area: Amazon
tm: TBD
last_updated: 2026-08-24
last_note: "LL response received; sent to business, follow up Wed."
next_due: 2026-08-26
tags:
  - deal
  - amazon
  - quick-commerce
---

# Uptown

## Tasks

- [x] Follow up with business on LL response 📅 2026-08-26 ✅ 2026-08-31

## Notes

2026-08-28 - Site code assigned: ZMS7.

2026-08-24 - LL response received. Sent to business — follow up Wed.

2026-08-17 - [Source: email] Two alt candidate sites in play alongside the 2020 28th St E negotiation, both surfaced in the "Quick Commerce: Uptown Minneapolis" thread w/ Jason Vangalis, Nathan Eady, Stephanie Meier (Amazon Corp Counsel, Alcohol), cc Claire Weber, Alan Oketcho, Jacob Sleight, Kristy Cole:

- **2118 4th Ave S, Minneapolis, MN 55404** — zoning RM3. General Retail Sales & Services and Grocery Store permitted up to 10k/20k SF respectively but MUST be in a mixed-use building; Package Delivery Service permitted up to 10k SF (same mixed-use condition); Liquor Store (off-sale) and Warehousing & Storage both **prohibited**. Nathan (Economic Development & Policy) also flagged three open policy questions to City of Minneapolis: (1) would a darkstore QC operation be classified as warehouse vs. "package delivery service, no on-site vehicle fleet"; (2) City's appetite for a customer lobby / retail commercial-zone use (Amazon designs support up to 51% customer shopping access); (3) whether alcohol (beer/wine) could run as an ancillary use to package delivery in zones where off-sale liquor store is itself prohibited.
- **4700-4850 Park Glen Rd, St. Louis Park, MN** — Jason sent this as a new candidate 2026-08-17. Nathan replied same day: AHJ is City of St. Louis Park, zoning Business Park. Warehouse permitted up to 60% of total building SF; loading/unloading areas must be 100 ft+ from surrounding residential parcels. Base zoning "looks pretty good" per Nathan, assuming co-tenants cover the non-warehouse balance.
- **Open:** Stephanie Meier asked Nathan whether the Park Glen Rd zoning allows alcohol sales specifically (Off-sale intoxicating liquor license). Reminder from Stephanie: MN requires a standalone liquor store with at least a vestibule open to the public for retail sales to sell alcohol — same constraint as the existing conference-room-as-vestibule approach discussed for the primary 2020 28th St E site.

2026-08-03 - LOI sent to local broker for review.

2026-08-03 - Site debrief. Second opportunity after ZMS3 cancellation (LL reversed post-LOI execution, increased scope of work to unacceptable CapEx/liability/risk). Market survey extensive but options remain limited — this is closest viable site currently available.

- Multi-tenant industrial bldg, previously medical office/med-tech. Proposed premises ~8,000 SF demised from a 21,175 SF Suite 108.
- Asking rent: $169,400/yr.
- Drivetime 12.07 min — longer than preferred, but comparable to GoPuff's service time.
- 1 drive-in door (inbound), 2 man doors (outbound).
- 226-space shared parking field; negotiating 10 exclusive stalls.
- Electrical: three 200A/208V panels + one 400A/480V panel.
- ~21' clear height, reinforced concrete floor, office sprinkler system, existing mop sink. HVAC tonnage and ADA restroom compliance TBC.
- Zoning: I-3. EDP moderate risk — supports primary QC use, but alcohol entitlement is the open issue. Minneapolis requires off-sale liquor store model w/ alcohol inventory physically separated from grocery; 2,000ft separation from existing off-sale liquor stores; 300ft from schools/churches. Team discussed repurposing existing conference room (outside proposed premises, direct access to ops area) as alcohol vestibule, similar to CA model.
- Topology to prep coverage maps for ZSM6 (LOI) and ZMP1 (lease negotiation) — check whether those, if launched 2026, could give interim coverage for Uptown.
- EDP to pull zoning map + review comparable zoning at existing GoPuff location (815 Cedar Ave) — GoPuff operating at risk on zoning there.
- Heading to Leadership escalation call for shortlist decision on drivetime + alcohol component.
- Next: JK proceeding with site negotiations (conference room inclusion + exclusive parking). JK's next action is to draft LOI.

Challenges flagged: drivetime validation, securing exclusive parking, confirming HVAC tonnage, confirming ADA restroom compliance, negotiating conference room into premises.
