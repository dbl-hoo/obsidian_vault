---
site_code: SOU3
aliases:
  - SKYY
deal_type: Lease
business_unit: SSD
status: Selected
tm: Mike Nevel
pcm: Alex Richardson (nalexari@amazon.com)
launch_date: 2027-01-01
start_date:
end_date:
local_broker: Powell Spears, JLL
area: Amazon
tags:
  - deal
  - amazon
last_updated: 2026-06-30
last_note: Followed up with LL on zoning letter and lease; follow up Monday 7/6.
---

## Tasks

- [ ] Follow up on zoning letter — hannah.schreck@louisville.gov 📅 2026-07-08
- [x] Follow up with LL on zoning letter and lease 📅 2026-07-06 ✅ 2026-07-06
- [x] Follow up with LL on lease 📅 2026-06-30 ✅ 2026-06-30

## Notes

2026-06-30 - Followed up with LL on zoning letter and lease. Follow up Monday 7/6.30
last_note: Followed up with LL on zoning letter and lease; follow up Monday 7/6.
---

## Tasks

- [x] Follow up with LL on zoning letter and lease 📅 2026-07-06 ✅ 2026-07-01
- [x] Follow up with LL on lease 📅 2026-06-30 ✅ 2026-06-30

## Notes

2026-06-30 - Followed up with LL on zoning letter and lease. Follow up Monday 7/6.

2026-06-22 - Alexandra Richardson is PCM. 3rd party architectural approval received. LL counsel: Greg Ehrhard, Stites & Harbison (https://www.stites.com/attorneys/t-gregory-ehrhard/). Revised lease sent to LL.

2026-06-12 - Brief call w/ Adam Thomas (LL). Lease can't sign until July — he's hoping for early July but may slip longer. Keith Seo's counsel expected to reach out re: how to handle title (their title company is slow). Adam working on ARC approval; UPS is also in the ARC (FYI).

2026-06-11 - Site code renamed from SKYY to SOU3.

2026-06-10 - CAR hasn't launched yet; supposed to launch soonish (TBD). Architectural review board exists, run by the airport authority — Adam Thomas. New PCM contact: Alex Richardson (nalexari@amazon.com); need to get the plans to her.

2026-06-09 - LL broker contact confirmed: Robert B. Walker SIOR, CCIM, Executive Managing Director, Commercial Kentucky — Cell: +1 502 298 9255 | Direct: +1 502 719 3254 | rwalker@commercialkentucky.com. Followed up on title report and base building plans.

2026-06-01 - Transitioned from Andrea Roy (departing KBC Nashville). Handoff per 2026-06-01 spreadsheet review.

Site: 8001 Air Commerce Drive, Louisville, KY. In for CAR; lease substantially complete. Keith Seo (legal) willing to proceed on transactions vetting the base building standard. Biggest issue is zoning: Louisville Airport Authority isn't filing the ZCL, and Amazon can't figure out how to do it themselves. Andrea planned to file it and eat the $50 fee — note that what comes back will not be on the correct form and will need cleanup before execution. Precon has not engaged on this site at all and has not reviewed any drawings.

**Key contacts:** Mike Nevel (TM) | Powell Spears, JLL (local broker) | Robert Walker, C&W (landlord's rep) | Adam Thomas, Louisville Airport Authority (landlord/seller)
