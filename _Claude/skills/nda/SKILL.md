...
**Field rules** (this table is the canonical spec for `KBC/NDA Log.md` — also applies when logging an NDA outside a full review, e.g. "log this NDA as signed"):

- **Type:** `Mutual` / `One-Way` / `—` (if not yet determined)
- **Status:** `Received` / `Reviewed` / `Signed` / `Dead`
- **Date Reviewed:** the date Jason reviewed it — **always populate this**
